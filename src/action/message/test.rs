...
fn test_given_message_action_when_it_is_executed_then_it_uses_the_messages_to_render_the_message_and_additonal_templated_values() {
    ...
    let state = {
        ...
        state.add_message(Rc::new(MessageBuilder::new("uuid1", "message1").pair("key1", "value1").build()));
        state.add_message(Rc::new(MessageBuilder::new("uuid2", "message2").pair("key2", "value2").build()));
        state
    };
    let responses = Rc::new(RefCell::new(Vec::new()));
    let message_action = {
        let response_sender = DummyResponseSender {responses: responses.clone()};
        let message = Template::compile("key1={{{messages.[0].values.key1}}} key2={{{messages.[1].values.key2}}}".to_string()).ok().expect("Failed to compile a handlebars template");
        let config_action = config::action::message::MessageActionBuilder::new("uuid", message)
                                .pair("message_num", Template::compile("we have {{context_len}} messages".to_string()).ok().expect("Failed to compile a handlebars template"))
                                .build();
        MessageAction::new(
            Rc::new(RefCell::new(Box::new(response_sender))),
            config_action
    ...
    if let &Response::Message(ref response) = responses.get(0).unwrap() {
        ...
        assert_eq!("we have 2 messages", message.get("message_num").expect("Failed to get an inserted key from a map"));
