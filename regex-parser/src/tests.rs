...
use syslog_ng_common::{LogMessage, Parser, ParserBuilder, ErrorKind, mock, GlobalConfig, SYSLOG_NG_INITIALIZED, syslog_ng_global_init};
...
fn test_parser_cannot_be_built_with_invalid_regex() {
    ...
    if let ErrorKind::MissingRequiredOption(value) = builder.build().err().unwrap().into_kind() {
