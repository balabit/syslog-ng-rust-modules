...
fn test_given_message_when_it_does_not_have_a_name_then_its_ids_contain_only_the_its_uuid() {
    ...
    assert_eq!(true, message.ids().into_iter().any(|x| x == uuid.as_bytes()));
    assert_eq!(1, message.ids().into_iter().count());
}
...
fn test_given_message_when_it_has_a_name_then_its_ids_contain_both_its_uuid_and_name() {
    ...
    assert_eq!(true, message.ids().into_iter().any(|x| x == name.as_bytes()));
    assert_eq!(2, message.ids().into_iter().count());
}
