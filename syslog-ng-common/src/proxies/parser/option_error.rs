     ...
     fn fmt(&self, formatter: &mut Formatter) -> Result<(), Error> {
         match *self {
             OptionError::MissingRequiredOption(ref name) => {
                 formatter.write_fmt(format_args!("At least one required option is missing. option_name={}", name))
             }
             OptionError::InvalidValue{ref option_name, ref value, ref expected_value} => {
                 formatter.write_fmt(format_args!("Invalid value in option. option_name={} value={} expected_value={}", option_name, value, expected_value))
             }
         }
     }
}
